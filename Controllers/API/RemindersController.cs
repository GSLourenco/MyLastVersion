...
namespace MvcApplication2.Controllers
{
    public class RemindersController : ApiController
    {
        ...
        public IEnumerable<Reminder> getAllReminders()
        {
            ...
            if (list.Any())
            {
                list = list.Where(r =>  DateTime.ParseExact(r.date+" "+r.time, "yyyy-M-d H:m",
        ...
        [BasicAuthenticationAttributeWithPassword,System.Web.Http.HttpGet]
        ...
        public HttpResponseMessage PostReminder(Reminder r)
        {
            ...
            if (reg_id == String.Empty) return this.Request.CreateResponse(HttpStatusCode.BadRequest, "User is not registered in GCM");
            int id = PictogramsDb.getContactId(contact, username);
            IEnumerable<Reminder> list = PictogramsDb.getAllReminders(id, username);
            if (!list.Any()) return this.Request.CreateResponse(HttpStatusCode.MethodNotAllowed);
